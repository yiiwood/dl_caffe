...
class MapAccuracyLayer : public Layer<Dtype> {
 public:
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_MAP_ACCURACY;
  }
...
template <typename Dtype>
class EuclideanLossMaskLayer : public LossLayer<Dtype> {
 public:
  explicit EuclideanLossMaskLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);

  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_EUCLIDEAN_LOSS_MASK;
  }
  virtual inline bool AllowForceBackward(const int bottom_index) const {
    return true;
  }
...
// Ydg
template <typename Dtype>
class MapHingeLossLayer : public LossLayer<Dtype> {
 public:
  explicit MapHingeLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}

  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_MAP_HINGE_LOSS;
  }
