...
class Layer {
 public:
  ...
  void PrintParamDataMinMax() {
    if ( blobs_.size() == 0) {
      //LOG(INFO) << layer_param_.name() << " does not have param_data!";
    }   
    for(int i = 0; i < blobs_.size(); ++i) {
      LOG(INFO) << layer_param_.name() << " param_data " << i 
                << ": min=" << blobs_[i]->min_data() << " max=" << blobs_[i]->max_data();
    }   
  }

  void PrintParamDiffMinMax() {
    if ( blobs_.size() == 0) {
      //LOG(INFO) << layer_param_.name() << " does not have param_diff!";
    }   
    for(int i = 0; i < blobs_.size(); ++i) {
      LOG(INFO) << layer_param_.name() << " param_diff " << i 
          << ": min=" << blobs_[i]->min_diff() << " max=" << blobs_[i]->max_diff();
    }   
  }
  ...
  void CheckParamData() {
    ...
    for(int i = 0; i < blobs_.size(); ++i) {
      blobs_[i]->check_data();
      LOG(INFO) << layer_param_.name() << " check_data " << i << ": OK";
    }   
  }
  ...
  void CheckParamDiff() {
    ...
    for(int i = 0; i < blobs_.size(); ++i) {
      blobs_[i]->check_diff();
      LOG(INFO) << layer_param_.name() << " check_diff " << i << ": OK";
    }   
  }
