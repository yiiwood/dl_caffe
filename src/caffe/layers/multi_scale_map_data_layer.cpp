...
MultiScaleMapDataLayer<Dtype>::~MultiScaleMapDataLayer<Dtype>() {
  this->JoinPrefetchThread();
}

template <typename Dtype>
void MultiScaleMapDataLayer<Dtype>::LayerSetUp(
  ...
  DLOG(INFO) << "Initializing MultiScaleMapDataLayer";
}

template <typename Dtype>
void MultiScaleMapDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
  const string& source = this->layer_param_.multi_scale_map_data_param().source();
  ...
  if (this->layer_param_.multi_scale_map_data_param().shuffle()) {
  ...
  if (this->layer_param_.multi_scale_map_data_param().rand_skip()) {
    unsigned int skip = caffe_rng_rand() %
        this->layer_param_.multi_scale_map_data_param().rand_skip();
  ...
  // 
  receptive_field_ = this->layer_param_.multi_scale_map_data_param().receptive_field();
  CHECK(receptive_field_ % 2);
  pad_ = receptive_field_ / 2;
  min_iou_ = this->layer_param_.multi_scale_map_data_param().min_iou();
  stride_ = this->layer_param_.multi_scale_map_data_param().stride();
  start_h_ = this->layer_param_.multi_scale_map_data_param().start_h();
  start_w_ = this->layer_param_.multi_scale_map_data_param().start_w();
  ...
  CHECK_GT(this->layer_param_.multi_scale_map_data_param().scale_vector_size(), 0) <<
    "scale_vector_size must be greater than 0";
  vector<float> tmp_s;
  for(int i = 0; i < this->layer_param_.multi_scale_map_data_param().scale_vector_size(); ++i) {
    tmp_s.push_back( this->layer_param_.multi_scale_map_data_param().scale_vector(i) );
  }
...
template <typename Dtype>
void MultiScaleMapDataLayer<Dtype>::DataReshape(vector<Blob<Dtype>*>* top, float scale) {
  int batch_size = this->layer_param_.multi_scale_map_data_param().batch_size();
...
template <typename Dtype>
void MultiScaleMapDataLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
void MultiScaleMapDataLayer<Dtype>::InternalThreadEntry() {
  ...
  ImageDataParameter scale_map_data_param = this->layer_param_.multi_scale_map_data_param();
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    //ReadLabel_IOU(lines_[lines_id_].second, resize_height_, resize_width_, pad_, 
    //  receptive_field_, min_iou_, label_mat);
    ReadLabel_CENT(lines_[lines_id_].second, resize_height_, resize_width_, pad_,
...
template <typename Dtype>
bool MultiScaleMapDataLayer<Dtype>::ReadData(string image_path, const Dtype* mean, 
...
template <typename Dtype>
bool MultiScaleMapDataLayer<Dtype>::ReadLabel_IOU(string label_path, int rs_height, int rs_width, int pad, 
  ...
  while (inf >> bleft >> btop >> bwidth >> bheight >> bcls) {
    ...
    for(int h = -rs_bheight_r; h <= rs_bheight_r; ++h) {
      for(int w = -rs_bwidth_r; w <= rs_bwidth_r; ++w) {
        if( IOU(rs_bch - rs_bheight_r, rs_bcw - rs_bwidth_r, rs_bch + rs_bheight_r, rs_bcw + rs_bwidth_r, 
          rs_bch + h - pad, rs_bcw + w - pad, rs_bch + h + pad, rs_bcw + w + pad) > min_iou_) {
...
template <typename Dtype>
bool MultiScaleMapDataLayer<Dtype>::ReadLabel_CENT(string label_path, int rs_height, int rs_width, int pad,
  ...
  while (inf >> bleft >> btop >> bwidth >> bheight >> bcls) {
    ...
    //if( abs(diff) < receptive_field_thres ) {
...
template <typename Dtype>
float MultiScaleMapDataLayer<Dtype>::IOU(int top1, int left1, int bottom1, int right1, 
...
template <typename Dtype>
bool MultiScaleMapDataLayer<Dtype>::FillDataLabel(cv::Mat &image_data, cv::Mat &label_data, 
...
template <typename Dtype>
void MultiScaleMapDataLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, vector<Blob<Dtype>*>* top) {
  InternalThreadEntry();
...
INSTANTIATE_CLASS(MultiScaleMapDataLayer);
