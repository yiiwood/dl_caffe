...
void MapDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  (*top)[0]->Reshape(
      this->layer_param_.map_data_param().batch_size(), datum_map.datum_channels(),
...
void MapDataLayer<Dtype>::InternalThreadEntry() {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    // jitter 
    if (jitter_) {
      //this->data_transformer_.Jitter(datum_map, true, 0.05, 8);
      this->data_transformer_.Jitter(datum_map, true, 0.2, 20);
    }
    // fill data and label
